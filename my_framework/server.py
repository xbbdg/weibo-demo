...
from my_framework.utils import log

from my_framework.routes import (
    error,
)
...
def response_for_path(request, route):
    ...
    log('response_for_path', response)
...
class WsgiRequest(BaseRequest):
    ...
    def __init__(self, environ):
        ...
        self.query = self.wsgi_query(query_str)
        log('请求 path 和 query', self.path, self.query)
        from gunicorn.http.body import Body
        body: Body = environ['wsgi.input']
        body = body.read()
        log('body', body, type(body))
        self.body = body.decode(encoding='utf-8')
        log('body decode', self.body)
    ...
    def form(self):
        body = urllib.parse.unquote_plus(self.body)
        log('raw body', self.body)
        log('parse unquote body', body)
        args = body.split('&')
        f = {}
        log('args', args)
        if body != '':
